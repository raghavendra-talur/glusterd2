...
func heketiDeviceAddHandler(w http.ResponseWriter, r *http.Request) {
	...
	fmt.Printf("rtalur %v is nodeid and %v is deviceName", nodeIDRaw, deviceName)

	deviceinfo.DeviceName = deviceName

	nodeID := uuid.Parse(nodeIDRaw)
	if nodeID == nil {
		log.Error("rtalur Could not parse nodeID provided")
	...
	if err != nil {
		log.WithError(err).Error("rtalur Could not find node in store")
	...
	if err != nil {
		...
		}).Error("rtalur failed to prepare device")
		restutils.SendHTTPError(w, http.StatusInternalServerError, err.Error())
		return
	}
	...
	if err != nil {
		log.WithField("error", err).Error("rtalur Failed to marshal the DeviceInfo object")
		restutils.SendHTTPError(w, http.StatusInternalServerError, err.Error())
		return
	}
	...
	if err != nil {
		log.WithError(err).Error("rtalur Couldn't add deviceinfo to store")
		restutils.SendHTTPError(w, http.StatusInternalServerError, err.Error())
		return
	}
